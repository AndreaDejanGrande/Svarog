...
{
	const char *job_id, *prevhash, *coinb1, *coinb2, *doncoinb1, *doncoinb2, *version, *nbits, *ntime;
	...
	doncoinb1 = json_string_value(json_array_get(params, 4));
	doncoinb2 = json_string_value(json_array_get(params, 5));
	merkle_arr = json_array_get(params, 6);
	if (!merkle_arr || !json_is_array(merkle_arr))
		goto out;
	merkle_count = json_array_size(merkle_arr);
	version = json_string_value(json_array_get(params, 7));
	nbits = json_string_value(json_array_get(params, 8));
	ntime = json_string_value(json_array_get(params, 9));
	clean = json_is_true(json_array_get(params, 10));

	if (!job_id || !prevhash || !coinb1 || !coinb2 || !doncoinb1 || !doncoinb2 || !version || !nbits || !ntime ||
	...
	sctx->job.doncoinbase = realloc(sctx->job.doncoinbase, sctx->job.doncoinbase_size);
