...
static enum algos opt_algo = ALGO_VCRYPT;
static int opt_vcrypt_n = 1024;
...
Options:\n\
  -a, --algo=ALGO       specify the algorithm to use\n\
                          vcrypt    vcrypt(1024, 1, 1) (default)\n\
                          vcrypt:N  vcrypt(N, 1, 1)\n\
                          sha256d   SHA-256d\n\
...
static void stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
	...
	work->donxnonce2 = realloc(work->donxnonce2, sctx->xnonce2_size);
        memcpy(work->donxnonce2, sctx->job.donxnonce2, sctx->xnonce2_size);

	/* Generate merkle root */
	sha256d(merkle_root, sctx->job.coinbase, sctx->job.coinbase_size);
	unsigned char doncoinbase_hash[64];
	sha256d(doncoinbase_hash, sctx->job.doncoinbase, sctx->job.doncoinbase_size);
	...
	for (i = 0; i < sctx->xnonce2_size && !++sctx->job.donxnonce2[i]; i++);
	...
	if (opt_debug) {
		...
		free(xnonce2str);
		applog(LOG_INFO, "DEBUG: checked before abin2hex for donxnonce");
		char *donxnonce2str = abin2hex(work->donxnonce2, work->xnonce2_len);
		applog(LOG_DEBUG, "DEBUG: job_id='%s' donextranonce2=%s ntime=%08x",
		       work->job_id, donxnonce2str, swab32(work->data[17]));
		free(donxnonce2str);
	}

	if (opt_algo == ALGO_VCRYPT)
...
static void *miner_thread(void *userdata)
{
	...
	if (opt_algo == ALGO_VCRYPT) {
		scratchbuf = vcrypt_buffer_alloc(opt_vcrypt_n);
		if (!scratchbuf) {
			applog(LOG_ERR, "vcrypt buffer allocation failed");
	...
	while (1) {
		...
		if (max64 <= 0) {
			switch (opt_algo) {
			case ALGO_VCRYPT:
				max64 = opt_vcrypt_n < 16 ? 0x3ffff : 0x3fffff / opt_vcrypt_n;
				break;
			case ALGO_SHA256D:
				max64 = 0x1fffff;
				break;
		...
		switch (opt_algo) {
		case ALGO_VCRYPT:
			rc = scanhash_vcrypt(thr_id, work.data, scratchbuf, work.target,
			                     max_nonce, &hashes_done, opt_vcrypt_n);
			break;
...
#ifdef HAVE_SYSLOG_H
	if (use_syslog)
		openlog("svarog", LOG_PID, LOG_USER);
#endif
